...
/// configured [`BlockSize`] in use. At maximum [`BlockSize`] capacity, each [`ContentReference`]
/// can store $2^255 * 255$ bytes of data (less some overhead).
///
...
///
/// As of right now, a [`BlockSize::small()`] can store 256KiB (ignoring some overhead), and
/// [`ContentLocation`] Data block references take up 41 bytes allowing 6,393 entries allowing up
/// to close to 400TiB in a single file entry. More can be used by using additional indirect blocks
/// or a large block size for the indirect reference block.
#[derive(Clone, Debug)]
pub enum BlockKind {
    Data,
