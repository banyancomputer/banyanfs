...
use crate::stores::{ApiSyncableStore, MemorySyncTracker};
...
pub type BrowserStore = ApiSyncableStore<WebFsDataStore, MemorySyncTracker>;

pub async fn initialize_browser_store(
    api_client: ApiClient,
) -> Result<BrowserStore, BrowserStoreError> {
    let store = WebFsDataStore::new().await?;
    let tracker = MemorySyncTracker::default();

    Ok(ApiSyncableStore::new(api_client, store, tracker))
}
